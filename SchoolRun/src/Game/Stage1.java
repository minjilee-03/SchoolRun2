...
public class Stage1 {
	...
	private ImageIcon backIc_2 = new ImageIcon("..//image//back_image//classback.jpg");
	//캐릭터 이미지
	...
	private String jellyStr = "000000011110000100100100100200120000100000003000222000001110000001000202000000200000001100010101010101010101010101010101010101010101010100000000003";
	private String fieldStr = "123412341234123412341234123402341234123412341230123412341234123412341234123412340034123412341234123412341234120412341234123412341234123410041234123412"; //150개
	private String tacleStr = "0000000000000000001002200011000000020000002000000000111000000000222000000001100000000022200000000022000000010000000220000020000000100001100000000000";
	//그 외 게임에 필요한 변수
	...
	int foots, face, foots2, face2; //foots2와 face2는 친구기능땜에 필요
	...
	class GamePanel extends JPanel {
		...
		public GamePanel() {
			...
			if(u1.isItem==3) { //생명이 두배
				c1.setHp(2000);
			} else if(u1.isItem==5) { //부스터 처음 속도값
				gameSpeed = 10;
		...
		new Thread(new Runnable() {
			...
			public void run() {
				while(over!=2) {
					...
					if(foots < nowField && !c1.isJump() && !c1.isFall() && !c2.isJump() && !c2.isFall()) { //캐릭터가 발판보다 위에 있고, 점프중이 아니고, 낙하중이 아닐 때
						c1.setFall(true);
						c2.setFall(true);
